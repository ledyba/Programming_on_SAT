...
　さて、プログラムの実行ステップごとに論理変数を増やすと言いましたが、どこまで増やせばよいのでしょうか？100ステップ？100000ステップ？100000000000000ステップ？

　答え：**無限回**

　なぜなら、brainfuckは無限ループするプログラムを簡単に書けるからです。ほら： `+[]`

　となると、こういったbrainfuckプログラムは無限のメモリが無いとbrainfuckを正しく実行することは出来ず、有限の世界に存在する我々のコンピュータではSATソルバーを使ってbrainfuckプログラムを最後まで（停止するまで）実行することが出来ないことがわかります。も

　同様の議論がチューリングマシンに対してもでき、「**SATはチューリング完全ではない**」ことがわかります。
